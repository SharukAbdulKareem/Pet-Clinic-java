 ...
 * @author sharuk
 */
@WebServlet(name = "VeterinarianSearch", urlPatterns = {"/VeterinarianSearch"})
public class VeterinarianSearch extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String recepName = request.getParameter("username");
        
        
        try (PrintWriter out = response.getWriter()) {
            try {
                List<MyVeterinary> findVets = myVeterinaryFacade.searchByVetenarianName(recepName);
                

                if (findVets != null) {
                    // Set the foundReport attribute for JSP to access
                    request.setAttribute("vetsList", findVets);
                } else {
                ...
                request.getRequestDispatcher("VeterinarianTable2.jsp").forward(request, response);
