...
public class StaffSearch extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            try {
                List<MyStaff> findStaff = myStaffFacade.searchByStaffName(staffName);
                

                if (findStaff != null) {
                    // Set the foundReport attribute for JSP to access
                    request.setAttribute("staffList", findStaff);
                } else {
                ...
                request.getRequestDispatcher("StaffTable2.jsp").forward(request, response);
